...
from creditmemo.fields import is_supplied
...
def _unset_core_element(properties, qname: str) -> None:
    """
    Remove one element from the core properties part, leaving it genuinely
    absent rather than present-and-empty or present-and-defaulted.

    **Not setting a property is not the same as leaving it unset.** The
    python-docx default template ships a *populated* core.xml — measured on
    python-docx 1.2.0, its children are ``dc:title`` (empty), ``dc:subject``,
    ``dc:creator`` = ``python-docx``, ``cp:keywords``, ``dc:description`` =
    ``generated by python-docx``, ``cp:lastModifiedBy``, ``cp:revision``,
    ``dcterms:created`` = ``2013-12-23T23:15:00Z``, ``dcterms:modified`` and
    ``cp:category``. A memo that skipped the setter inherited ``python-docx``
    as its Author and 23 December 2013 as its creation date.

    Nor can the setter be given ``None``: ``cp.created = None`` raises
    ``ValueError: property requires <type 'datetime.datetime'> object``, and
    ``cp.author = None`` writes the four characters ``None``. So the element is
    deleted. Measured after deletion: the file still opens, and python-docx
    reads the property back as ``''`` / ``None``.

    ``qname`` is a prefixed name python-docx's ``qn`` resolves —
    ``"dcterms:created"``, ``"dc:creator"``, ``"dc:title"``.
    """
    from docx.oxml.ns import qn
    ...
    if element is None:          # pragma: no cover - python-docx internals moved
        return
    node = element.find(qn(qname))
    if node is not None:
        element.remove(node)


def _set_core_text(properties, attribute: str, qname: str, value) -> None:
    """
    Set one core *text* property from a deal field, or leave it absent.

    The same posture :func:`parse_prepared_date` takes for a date the package
    cannot read: a value that states nothing is not written, not substituted
    for, and not raised on — the property is simply not there, and a reader of
    the loan file sees an empty Author rather than a wrong one.

    ``None``, ``""`` and whitespace are absence, decided by
    :func:`creditmemo.fields.is_supplied` so this agrees with the other 16
    optional-string fields rather than inventing a second rule. ``deal_name``
    and ``prepared_by`` are declared ``str`` and neither is validated, so a
    ``None`` reaches here through an ordinary incremental build — measured on
    the 0.2.2 wheel, ``prepared_by=None`` wrote
    ``<dc:creator>None</dc:creator>`` and ``deal_name=None`` wrote
    ``<dc:title>None</dc:title>``.

    A non-``str`` that states something is written through ``str()``: the field
    is typed ``str`` and the property is text, and refusing here would be the
    input validation this release does not add.

    The memo *body* still renders ``None`` for these fields. That predates this
    release, is disclosed, and its fix is 0.3.0; what 0.2.2 must not do is
    promote it into the document metadata a DMS indexes on.
    """
    if not is_supplied(value):
        _unset_core_element(properties, qname)
        return
    setattr(properties, attribute, value if isinstance(value, str) else str(value))
...
def _set_core_properties(doc, deal: DealProfile) -> None:
    ...
    ``author`` and ``title`` were set unconditionally through 0.2.2, on the
    reasoning that both sources are required ``str`` fields. Neither is
    validated, so neither is reliably a string: measured on the 0.2.2 wheel,
    ``prepared_by=None`` wrote ``<dc:creator>None</dc:creator>`` and
    ``deal_name=None`` wrote ``<dc:title>None</dc:title>``. A loan file whose
    Author is the literal word ``None`` is this package's doing, and 0.2.2 is
    the release that started writing these properties at all. They are now set
    only from a value that states something, and otherwise left absent — see
    :func:`_set_core_text`. The dates are set only from a ``prepared_date``
    this package can read; see :func:`parse_prepared_date`.
    ...
    _set_core_text(properties, "author", "dc:creator", deal.prepared_by)
    _set_core_text(properties, "title", "dc:title", deal.deal_name)

    prepared = parse_prepared_date(deal.prepared_date)
    if prepared is None:
        _unset_core_element(properties, "dcterms:created")
        _unset_core_element(properties, "dcterms:modified")
        return
