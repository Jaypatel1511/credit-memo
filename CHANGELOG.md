...
The suite goes from **427 tests to 523**, all measured: 427 at `c21e109`
(the 0.2.1 merge), 500 at `56bd00b` (the build commit, before the hostile
audit), 523 on this commit, all passing with `CREDITMEMO_REQUIRE_DOCX=1`. Every gate added below was watched to fail against
...
no required field**. Nothing an existing caller does stops working. Most of the
fixes change what a memo *says*; two change only the Word file's properties, and
one changes only a gate.
...
sentence. `Stable` fired on exact equality of the two endpoints — bare `>` and
`<` with a fall-through, so the equality condition was **zero-width**. An
earlier draft of this entry called it "a ±$1 band"; there was no tolerance in
the 0.2.1 code and there is none now.
...
**A non-finite revenue endpoint produced an incoherent sentence, and in one
case a false one.**

Found by the hostile audit of this release. Measured on this branch before the
fix:

    | Revenue | $nan | N/A | $nan |
    **Revenue, Year -2 vs Most Recent:** Unchanged — Most Recent revenue equals
    Year -2 revenue by less than the row above can show; both are written $nan.

    | Revenue | $nan | N/A | $3.00MM |
    **Revenue, Year -2 vs Most Recent:** Unchanged — Most Recent revenue of
    $3.00MM equals Year -2 revenue of $nan.

`nan != nan` is `True`, so the `Unchanged` fall-through caught it and the
sub-precision branch — whose verb is *equals* — wrote it out. The second line
**states that $3.00MM equals $nan**, which is a false sentence in the one place
this release exists to make unfalsifiable. `inf` went the other way and
produced a grammatical sentence quoting `$infMM`.

**It is not a regression**, and that is not a reason to defer it: 0.2.1
rendered the same input as *"Stable — revenue has been stable over the
historical period"*, which is worse. Nor is it hypothetical. A blank cell read
out of a spreadsheet of borrower financials through pandas is
`numpy.float64(nan)`, **not** `None` — measured, `pd.DataFrame({"rev": [1e6,
None]})["rev"][1]` — so the missing-endpoint guard never sees it. That is the
exact route the R20 comment in `sections/financial.py` already describes.

**A non-finite endpoint now renders no comparison sentence at all**, the same
silence a missing endpoint already produces. Silence asserts nothing. No input
validation was added and nothing raises: validating `revenue_y1` is a schema
change and is 0.3.0.

Which types the predicate was checked against, since `math.isfinite` is not
universal: `int` (including `10**30`), `float`, `bool` and `decimal.Decimal`
all answer — `Decimal("NaN")` and `Decimal("Infinity")` answer `False`, which
is the case a reader would expect a float-shaped guard to miss. So do
`numpy.float64` and `numpy.int64`, which is what pandas hands over.
`pandas.NA` and `pandas.NaT` are the two that make `math.isfinite` *raise*, so
the call is wrapped and an unjudgeable value is treated as non-finite; both
already raise earlier than this function in a full render — `money(pd.NA)`
raises *boolean value of NA is ambiguous* and `money(pd.NaT)` raises *bad
operand type for abs()*, and the Revenue table row is built before the
sentence — so nothing about their end-to-end behaviour changes.

**`prepared_by=None` wrote the Word Author as the literal string `None`.**

Also from the hostile audit. Measured on this branch before the fix:

    prepared_by=None  -> <dc:creator>None</dc:creator>
    deal_name=None    -> <dc:title>None</dc:title>
    prepared_by=''    -> <dc:creator></dc:creator>

`prepared_by` and `deal_name` are declared `str` and neither is validated, so a
`None` arrives through the ordinary field-by-field build. The memo *body* has
rendered `None` for these fields since before this release and that is
disclosed — but **0.2.2 is the release that promotes it into the document
metadata a DMS indexes on**, so a loan file whose Author is literally `None` is
this release's doing.

A `None`, empty or whitespace-only value now leaves the core property
**absent**, the same posture an unreadable `prepared_date` already takes.
Nothing is validated, nothing is substituted, nothing raises. "Absent" again
had to mean the element is removed rather than the setter skipped: skipping it
leaves python-docx's template default, `python-docx`, which is the false Author
this release exists to remove. Absence is decided by
`creditmemo.fields.is_supplied`, so this agrees with the rule the other 16
optional strings already follow instead of inventing a second one.

The body still renders `None`. That is older than this release, is disclosed
below, and is 0.3.0; a gate holds it so the disclosure cannot go stale.

**The gate against a hardcoded version asserted one spelling of it.**

`test_g16_no_shipped_module_writes_the_version_as_a_literal` read

    assert f'"{creditmemo.__version__}"' not in text

which rejects only the double-quoted form. The audit replaced
`version_footer()` with `return f"Generated by credit-memo 0.2.2"` — an
*interpolated* literal — and **this gate stayed green**; only the companion
sentinel gate reddened. That is the G14 shape this package has now hit seven
times: a gate asserting one spelling of a value where the property is what
matters.

The gate now parses each shipped module and rejects the version inside any
string the module *evaluates*, in any quoting and inside an f-string, with
docstrings and comments excluded so a module can still name the release it
describes in prose. Re-run against the audit's own mutation it goes red — 2
failed, this gate and the sentinel gate — and the old assertion, evaluated
against the same mutated file, still returns green. A companion gate feeds the
scanner a throwaway module so it cannot become vacuous.

**An ungated version literal in the README (audit finding 8).** README.md
shows the footer as `Generated by credit-memo 0.2.2`, which goes silently stale
at 0.3.0 — and this package has shipped nine false version attributions in
prose before. A gate now holds every `Generated by credit-memo <version>`
example in README.md to the installed `__version__`. It is scoped to the
example on purpose: sentences *about* the 0.2.2 release stay 0.2.2 forever,
because they are history.

**Three claims in the shipped documentation were false.**

A false claim in shipped docs stops a release in this portfolio, so all three
were corrected rather than carried.

1. **The README described a date parser that does not exist.** It said
   `prepared_date` is read when written as `2026-05-06`, `2026/05/06`,
   `May 6, 2026`, `6 May 2026`, "or those with a time". Measured:
   ...
   **Only the ISO forms take a time.** It fails safe — the property is left
   unset, never guessed — so nothing wrong reached a document, but the prose
   was false. The prose is now a table of every documented spelling and whether
   it is read, and a gate reads those rows out of README.md and holds each one
   against `parse_prepared_date` in **both** directions. `PREPARED_DATE_FORMATS`
   was **not** widened: that changes the metadata a memo carries for an input
   that produces none today, which is a behaviour change and is 0.3.0.

2. **This entry invented a tolerance that does not exist.** It said *"`Stable`
   fired on exact equality of the two endpoints, a ±$1 band."* The first clause
   is right and the appositive is not: the 0.2.1 implementation is bare `>` and
   `<` with a fall-through, so the condition is zero-width. Corrected above.

3. **The README overclaimed that the sentence can never contradict the table.**
   It said *"there is no reading of the memo in which the sentence contradicts
   the table."* Falsifiable at the MM/dollar crossover:

       revenue_y1=999_999.996, revenue_y3=1_000_000

       | Revenue | $1,000,000 | N/A | $1.00MM |
       **Revenue, Year -2 vs Most Recent:** Higher — Most Recent revenue of
       $1.00MM is higher than Year -2 revenue of $1,000,000.

   The sentence is **true** — the gap is four tenths of a cent — but a credit
   officer reads `$1.00MM` and `$1,000,000` as the same number. The guard is
   `first != last and first_text == last_text`, and here the two strings differ
   while denoting the same reader-visible quantity. **The claim was narrowed,
   not the guard widened**: widening needs a definition of when two cells in
   different units denote the same quantity to a reader, which is design work
   and is 0.3.0. The README now scopes the claim to endpoints that render in
   the same unit and states the residual case in full, and a gate holds both
   the rendering it quotes and the disclosure itself.
...
**A non-finite figure still renders as `$nan` in the Revenue row.** The
comparison sentence is now silent for such an endpoint, but
`creditmemo.money.money` still writes `$nan` / `$infMM` into the table cell.
That is the formatter's rendering of a non-finite magnitude and changing it is
the formatter sweep 0.3.0 already owns. A cell reading `$nan` is visibly not a
dollar figure; the sentence was a *claim*, which is why the two were separated.

**The memo body still renders `None` for an unstated `prepared_by`,
`deal_name` or `prepared_date`.** Older than this release and already disclosed
as a 0.2.x limitation. 0.2.2 fixes only the document metadata, which is the
part this release introduced. A gate holds the body's current behaviour so the
disclosure cannot go stale.

**`FinancialData` figures of type `Decimal` at or above $1,000,000 raise.**
Found while checking which types can reach the non-finite guard, and reported
rather than fixed: `creditmemo.money.money` computes `magnitude / MM_MIN` with
`MM_MIN` a `float`, and `Decimal / float` raises
`TypeError: unsupported operand type(s)`. Below the crossover the same value
renders correctly. This is pre-existing — it is in the published 0.2.1 wheel
too — it fails loudly rather than silently, and the fix belongs to the
formatter sweep. **0.3.0.**
...
- **Widening `PREPARED_DATE_FORMATS`** so a time can follow the non-ISO
  spellings. The README was narrowed to the parser instead. It is a behaviour
  change — an input that produces no metadata today would start producing
  some — and it is **0.3.0**.
- **Widening the sub-precision guard to compare cells as quantities** rather
  than as rendered strings, which is what the MM/dollar crossover case would
  need. It requires a definition of "these two cells denote the same figure"
  across unit formats, and that definition interacts with the unit-consistency
  plan already recorded for 0.3.0. **0.3.0**, and recorded there.
- **Input validation on the revenue fields.** The non-finite fix is silence,
  not a refusal: rejecting a `nan` at construction is a schema change and would
  break a caller who builds a `FinancialData` field by field off a sheet.
- **`cp.lastModifiedBy`, `cp.revision`, `cp.category`, `cp.subject` and
  `cp.keywords`**, which python-docx's template leaves empty and which this
  release still does not set. Nothing false is written into any of them; the
  scope here was the properties that carried a false value.

## [0.2.1] — 2026-09-08

Everything in this entry was measured on the code in this repository with the
README quickstart deal, on CPython 3.10.12 with python-docx 1.2.0, and every
figure in it was re-derived on that interpreter before release. An earlier
draft of this line said Python 3.12; no 3.12 interpreter exists on the machine
the measurements were taken on, so the version was asserted rather than read.
CI runs the suite on 3.9, 3.10, 3.11, 3.12, 3.13 and 3.14 -- the matrix used to
stop at 3.12 while this line claimed six versions and `requires-python`
admitted all six; the matrix moved. The suite goes from **86 tests to 427** — 86 measured on
`origin/main`, 427 on this commit. Every gate below was watched to fail against
a deliberate mutation before being trusted; the mutations are recorded in the
gates' own docstrings, with the exact command and the number of tests that
reddened. A red-proof records that failure count and not the number of tests
that passed alongside it: the failure count is the evidence, while a passed
count is a running total of the rest of the file and goes stale on the next
commit that adds a test. Two of them had already gone stale by the time this
entry was written, and an earlier draft of this paragraph said 340. Where a
suite total is the point it is anchored to the commit it was taken at — "at
2031b0a the suite was 284 passed" — which cannot drift.
...
both ways: removing it changes no test result, and
`test_g12_zero_is_the_one_number_both_conventions_agree_on` passes either
way. It is gone and that test stays -- the property is real, the clause was not
producing it. It was introduced during 0.2.1's own development, alongside the
band it sat above, so it never reached a released version.
...
generated by 0.1.0 or 0.2.0 for microenterprise or small-business deals contain
wrong figures; regenerate them.** (Both of those are the released versions that
carry the unconditional divide — `sections/borrower.py` at one decimal,
`sections/financial._fmt` at two, in 0.1.0 as in 0.2.0. 0.2.1 is the release
that fixes it, and an earlier draft of this entry named 0.2.1 among the
affected versions.)

**There is a third form below the cents form, and it is the one that carries
the release's central property.** A magnitude at or above half a cent always
carries a non-zero digit in the dollars-and-cents form, so it cannot print as
the zero string. Below half a cent there is no honest cent figure to print, and
rounding to `$0` would reintroduce exactly the defect above at a smaller
magnitude — a non-zero figure rendered as a stated zero. So the magnitude is
stated as a **bound** rather than a value: `<$0.01`, or `>-$0.01` below zero,
each a true inequality about the number. Both reach memos:

    | Origination Fee | 0.00% (<$0.01) |
    | Net Income | $0 | <$0.01 | >-$0.01 |

That last row is the point of the form: a stated zero, a positive sub-cent
figure and a negative sub-cent figure are three distinguishable strings, where
in 0.2.0 all three printed `$0.00MM`. The complete set of strings
`creditmemo.money` emits for a supplied figure is `$0`, `<$0.01`, `>-$0.01`,
the cents form (`$4,999`, `$4,999.60`, `-$4,000`) and the `$MM` form
(`$2.50MM`, `-$100.00MM`) — plus `N/A` for a figure the caller did not supply,
and the loan amount's two-unit `$2,500,000 ($2.50MM)`. Gated by G14, whose
sweep measures three renderings per dollar field — absent, zero, and a small
non-zero that must differ from zero.
...
rule is right, the reason was not.

The replacement — "an optional string is *always* rendered behind a label the
package supplies" — was quantified over the same 16 fields while enumerating
15, and is false for the one it left out. `DealProfile.deal_summary` is
reproduced as bare prose in the Executive Summary: no heading, no row label, no
bold prefix. So the reason now stands one step further back, where it holds for
all 16: an empty or whitespace-only string states nothing, so there is nothing
of the caller's to reproduce, and whatever the memo prints in its place is the
package's own output standing in for content. For 15 of the 16 that is a label
left standing with nothing after it — `| Anticipated Closing |  |`, `###
Mission` over a blank line — which is the harm the rule was written for. For
`deal_summary` there is no label to orphan and the cost is only a blank line;
it is inside the rule because the rule is uniform, and uniformity is what is
gated: `""` and `"   "` must render byte-identically to `None` for every one of
the 16, whatever shape that field takes. The full enumeration by rendering
shape is in `creditmemo.fields.is_supplied`. Both gates discover the 16 from
the annotations, so the enumeration is documentation and cannot silently become
the contract.

The gate `fields.is_supplied` cited for the corrected reason,
`test_g9_the_optional_string_rule_is_stated`, still carried the *superseded*
reason in its own docstring — the correction landed in two of the three places
that state it, and the citation pointed at the third. It now states the same
reason as the other two, and says which of its siblings holds the property over
all 16 fields.
...
- **A change too small for the rendered unit is invisible in a table and still
  visible to the revenue trend.** Above the `$MM` crossover:
  ...
  Two cells that read the same under a sentence saying they differ. This is the
  *shape* R24 closed, and it is not the same defect: there both figures were
  false — a $4,999 revenue printed as `$0.00MM`, a stated zero's own string —
  and here both cells are honest roundings of their inputs and the trend is
  honestly computed on the raw values. Nothing in the memo is wrong; the table
  is simply coarser than the comparison.

  **It is inherent to rounding, at any precision, and it is not fixable by
  moving the crossover.** Pick any unit and there is a change too small for it
  to show while a comparison of raw values still has a sign; at exact cents the
  pair would be a fraction of a cent apart. R24 chose where the invisible band
  sits — from `$1,000,000` up the `$MM` form is accurate to 0.5% or better — it
  did not abolish the band, and abolishing it is not available. The trend is
  computed on raw values *deliberately*: computing it on the rendered strings
  would make the memo's one analytical claim a function of the formatter, and
  would report "stable" for a real change.

  `test_g14_the_revenue_table_and_the_revenue_trend_agree` gates the property
  below the crossover, where R24's defect lived, and its docstring now says so
  rather than quantifying over all magnitudes.
- **One table can span the unit crossover, including across the comparison the
  table exists for.** Measured:
  ...
  Every cell is true. But assets against liabilities is the one comparison the
  Balance Sheet Summary exists to support, and the unit switch lands between
  the two cells, so the reader has to convert before they can compare. The
  second shape is the same thing inside one metric's own time series: a row
  that changes units mid-row while the underlying series barely moves.

  Mixed units within a table were accepted knowingly in R24 as the price of not
  printing a false figure, and that trade stands — a coarse-but-true figure
  beside an exact one is better than a figure that reads as zero. What was not
  weighed is that the mixing can land inside a comparison rather than merely
  between unrelated rows.

  **The planned direction for 0.3.0 is unit consistency within a table: if any
  figure in a table falls below the crossover, the whole table renders in
  dollars.** That removes both shapes without ever collapsing a figure — the
  dollars form is exact at every magnitude, so consistency is bought by
  rendering the *large* figures more precisely, never the small ones less. It
  is recorded here as the direction and is not implemented in 0.2.1: it changes
  figures an IC reads in every table that mixes units, and belongs with the
  gate coverage that goes with it.
- **The ratio, rate and price formatters still carry R24's property.** R24
  ruled on dollar magnitudes and `creditmemo/money.py` is the whole of that
  ruling. The non-money formatters were not swept, and each rounds a small
  non-zero to the characters a stated zero produces. Measured:

  | Site | Input | Renders |
  |------|-------|---------|
  | `sections/financial._fmt_ratio` (DSCR, current ratio, debt/equity, the three projected DSCRs) | `0.001` | `0.00x` |
  | `fields.rate` (interest rate, both sections) | `0.00001` | `0.00%` |
  | `sections/financial._fmt_ratio` for `ltv`, one decimal | `0.0001` | `0.0%` |
  | `sections/transaction`, `min_dscr_covenant` | `0.001` | `Minimum DSCR of 0.00x` |
  | `sections/transaction`, `max_ltv`, one decimal | `0.0001` | `Maximum LTV of 0.0%` |
  | `sections/transaction`, `origination_fee_pct` | `0.00001` | `0.00%` |
  | `sections/transaction`, `cde_fee_rate`, one decimal | `0.0001` | `0.0%` |
  | `sections/transaction`, `credit_price` | `0.001` | `$0.00/$1` |

  A DSCR is the figure an IC reads first, and `0.00x` for a supplied `0.001` is
  the dollar defect in the non-dollar half of the surface. It is disclosed
  rather than patched for the reason `_fmt_ratio`'s own docstring gives: fixing
  it means changing numbers an IC reads across seven sites, and it needs the
  gate sweep G14 gave the money surface. `credit_price` deliberately does not
  go through `creditmemo.money` — it is dollars per dollar of credit, a price
  in the unit the NMTC market quotes, not a magnitude in a column of magnitudes
  — and carries the same latent property at two decimals. **The formatter sweep
  that covers ratios, rates and prices is 0.3.0 work.** How much of it is real
  in practice differs by field: a 0.001 DSCR or a 0.00001 interest rate is an
  unusual input, and a `cde_fee_rate` at one decimal loses the difference
  between 2.04% and 2.0% on every NMTC deal.
- **Required (non-`Optional`) strings have no absence rule at all, and render
  two different ways in one memo.** The `""`-is-absence rule and
  `fields.is_supplied` govern the 16 `Optional[str]` fields. `deal_name`,
  `prepared_by`, `prepared_date` and the required `BorrowerProfile` strings are
  not optional, so the package assumes they were supplied and interpolates them
  as they are. Measured, with `deal_name=None`, `prepared_date=None`,
  `prepared_by="   "` and `borrower.name="  "`:
  ...
  `None` reaches the memo as the four characters `None` — as the memo's own
  title, and as the date of an IC memo — while whitespace reaches it as
  whitespace. Nothing raises. `fund_name`, which *is* optional, correctly reads
  `N/A` three lines above. Neither shape is defensible, and they are not the
  same failure: `None` prints a Python repr where a value belongs, whitespace
  prints a label with nothing after it. Closing this means either validating
  the required strings at construction — which rejects input the package
  accepts today — or giving them a placeholder, which is a rendering change to
  every memo that has one. Both are 0.3.0 decisions. A caller who assembles a
  `DealProfile` field by field, the route R20 exists for, is the one who hits
  it.
